#!/bin/sh
# Three routers in a row, end devices on each end.  Every virtual
# device/router live in their own network namespace.  Only R1 and R3 run
# mrouted and tunnel all multicast over the built-in IP-IP tunnel.
#
# NS1         NS2              NS3              NS4              NS5
...
print "Creating mrouted.conf for R1 ..."
cat <<EOF > "/tmp/$NM/r1.conf"
no phyint
phyint eth1 enable
#phyint eth2 enable
tunnel 10.0.1.1 10.0.2.2 metric 1 rate-limit 0
EOF
cat "/tmp/$NM/r1.conf"

print "Creating mrouted.conf for R3 ..."
cat <<EOF > "/tmp/$NM/r3.conf"
no phyint
#phyint eth5 enable
phyint eth6 enable
tunnel 10.0.2.2 10.0.1.1 metric 1 rate-limit 0
EOF
cat "/tmp/$NM/r3.conf"

print "Starting mrouted ..."
nsenter --net="$NS2" -- ../src/mrouted -i NS2 -n -p "/tmp/$NM/r1.pid" -f "/tmp/$NM/r1.conf" -l debug -d all -u "/tmp/$NM/r1.sock" &
echo $! >> "/tmp/$NM/PIDs"
...
echo $! >> "/tmp/$NM/PIDs"
nsenter --net="$NS3" -- tshark -lni eth3 -w "/tmp/$NM/r2.pcap" 2>/dev/null &
echo $! >> "/tmp/$NM/PIDs"
nsenter --net="$NS4" -- tshark -lni eth5 -w "/tmp/$NM/r3.pcap" 2>/dev/null &
echo $! >> "/tmp/$NM/PIDs"
nsenter --net="$NS5" -- tshark -lni eth0 -w "/tmp/$NM/ed2.pcap" 2>/dev/null &
echo $! >> "/tmp/$NM/PIDs"
...
if ! nsenter --net="$NS1"  -- ./mping -s -d -i eth0 -t 5 -c 10 -w 15 225.1.2.3; then
    ...
    dprint "DVMRP Status $NS4"
    nsenter --net="$NS4" -- cat /proc/net/ip_mr_vif
    nsenter --net="$NS4" -- cat /proc/net/ip_mr_cache
    nsenter --net="$NS4" -- ../src/mroutectl -u "/tmp/$NM/r3.sock" show compat detail
